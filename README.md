...
Just-In-Time makes several AI passes: the first labels every dollar amount in the justification; the second matches each against the spreadsheet; a third audits not-found values with a fresh match attempt; a fourth groups mismatches by root cause (consolidated to roughly four groups); and a final pass writes a short, neutral, plain-language explanation for each group — naming the specific conflicting figures so the discrepancy is concrete, but with no judgment or corrective language yet. The mismatch-grouping pass retries up to 3 times if it drops an item while grouping; if it still can't place every item after those retries, the leftover item(s) are added as their own individual group rather than failing the whole verification run — the step's summary notes when this happens (e.g. "5 groups identified (1 added individually after grouping missed it)").

If any findings remain, a chat modal opens and walks through each finding one at a time. Before the very first finding, a one-time introductory message ("I am the Just-In-Time AI Assistant...") appears for 3 seconds — summarizing whether the review ahead covers hard-to-find items, discrepancies, or both — during which the input, Send, Ignore, and "Got it" controls are locked; this intro is only shown once per fresh review, not on resume. Not-found items (titled "Hard to Find Items" on the summary card) are informational only: the assistant lists what it had trouble finding and suggests double-checking anything that looks important, with a single "Got it" acknowledgement to move on — no judgment call required, and they default to the dismissed/collapsed state in the final summary. Mismatch groups open with the assistant describing the neutral, suspected discrepancy and asking the user to confirm or correct it in a resizable, multi-line message box (Shift+Enter for a newline, Enter to send), or the user can click "Ignore Issue" to skip it immediately without discussion. The user replies in plain text, with the budget justification and spreadsheet — and a summary of any findings already resolved earlier in the same review, so the assistant understands references back to them — passed along as context so the assistant can check the user's explanation against the source documents rather than take it at face value. The assistant's role in this conversation is limited to identifying whether something is a real issue or not — it will not offer to draft or write corrected justification text itself. The conversation defaults to closing and marking the finding a real issue after any reply; it only keeps going if the user asks a genuine question or offers clarifying context, and only closes as "not a concern" if the user's reply states or implies it isn't actually a problem — the user drives whether the conversation continues, not the assistant. As a hard rule enforced in code rather than left to the model's judgment, a reply containing a "?" always keeps the conversation open, overriding whatever the model itself decided. On close, the assistant writes a standalone, professionally worded resolution summary — distinct from its conversational chat reply, and phrased around what the documents show rather than referencing "the user" — that states the issue, the relevant context, and the fix or next step, folding in anything learned during the conversation rather than just repeating the original finding. This resolution fully replaces the original neutral finding text in the final summary rather than appearing alongside it, avoiding a redundant one-line-then-paragraph repeat. Each chat message fades and drops gently into place as it arrives. Once a finding is confirmed real or dismissed (via chat resolution or Ignore Issue), no separate assistant reply is shown — instead a checkmark badge ("Marked as Issue" or "Not an Issue") and the written resolution appear immediately together, then sit on screen for 3 seconds (input locked) before anything moves on, so there's time to actually read it. Advancing to the next finding then slides the conversation pane out and the new one in; on the very last finding, instead of that slide, the assistant adds a closing line ("That's it! I'll write a summary of the findings...") and pauses briefly before the modal closes. Progress is saved to `localStorage` as you go, so closing the tab or reloading the page mid-review surfaces a "Continue Review" or "View Summary" prompt (once both files are re-selected) that picks up where you left off. Once every finding is resolved, the full per-finding conversation log is also recorded as its own step in the progress log above (labeled "Reviewing findings," with a "Details" toggle per finding), alongside the other AI-pass steps. A simplified summary then renders: real mismatches as color-coded cards (yellow for values missing from the spreadsheet, red for mismatches) with the resolution text shown up front and the itemized dollar-value diffs collapsed behind a "See details" toggle, and dismissed/hard-to-find/ignored items collapsed under a "Reviewed — no action needed" toggle. A marked-up copy of the justification document can be downloaded with problem values highlighted — mismatches dismissed or ignored during the chat are left out of this download, since they're no longer considered errors; hard-to-find (not-found) items are still included since those were never judged one way or the other.
