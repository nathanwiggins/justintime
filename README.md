...
If any findings remain, a chat modal opens and walks through each finding one at a time. Not-found items (titled "Hard to Find Items" on the summary card) are informational only: the assistant lists what it had trouble finding and suggests double-checking anything that looks important, with a single "Got it" acknowledgement to move on — no judgment call required, and they default to the dismissed/collapsed state in the final summary. Mismatch groups open with the assistant describing the actual suspected discrepancy (the same explanation that would appear on the summary card) and asking the user to confirm or correct it. The user replies in plain text, with the budget justification and spreadsheet passed along as context so the assistant can check the user's explanation against the source documents rather than take it at face value. The back-and-forth continues until the assistant is confident it has reached agreement with the user, at which point it states its conclusion — noting the mismatch as real, or that it will disregard the finding based on what the user explained. Progress is saved to `localStorage` as you go, so closing the tab or reloading the page mid-review surfaces a "Continue Review" or "View Summary" prompt (once both files are re-selected) that picks up where you left off. Once every finding is resolved, a simplified summary renders: real mismatches as color-coded cards (yellow for values missing from the spreadsheet, red for mismatches), and dismissed/hard-to-find items collapsed under a "Reviewed — no action needed" toggle. A marked-up copy of the justification document can be downloaded with problem values highlighted.
