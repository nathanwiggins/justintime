...
- **Live document preview during review** — for `.docx` uploads, the chat modal shows the justification side-by-side with the conversation (via `js/doc-preview.js`, reusing `Highlighter`'s value/context matching). Every flagged figure is subtly marked throughout the document; the figure(s) tied to whichever finding is currently being discussed are highlighted more prominently and auto-scrolled into view as the chat steps through findings. `.pdf` justification uploads show a "preview isn't available" message instead, since only `.docx` can be rendered this way today.
...
If any findings remain, a chat modal opens and walks through each finding one at a time. Not-found items (titled "Hard to Find Items" on the summary card) are informational only: the assistant lists what it had trouble finding and suggests double-checking anything that looks important, with a single "Got it" acknowledgement to move on — no judgment call required, and they default to the dismissed/collapsed state in the final summary. Mismatch groups open with the assistant describing the neutral, suspected discrepancy and asking the user to confirm or correct it. The user replies in plain text, with the budget justification and spreadsheet passed along as context so the assistant can check the user's explanation against the source documents rather than take it at face value. The conversation closes as soon as the user gives a clear affirmation or defers to fixing it later — it only continues if the user asks a genuine question or offers new context that changes the picture. On close, the assistant tags the finding as a real issue or not a concern and writes a standalone, professionally worded resolution summary — distinct from its conversational chat reply — that states the issue, any context the user provided, and the fix or next step, folding in anything it learned from the user rather than just repeating the original finding. This resolution becomes a second, distinct paragraph shown under the neutral finding. Progress is saved to `localStorage` as you go, so closing the tab or reloading the page mid-review surfaces a "Continue Review" or "View Summary" prompt (once both files are re-selected) that picks up where you left off. Once every finding is resolved, a simplified summary renders: real mismatches as color-coded cards (yellow for values missing from the spreadsheet, red for mismatches) with the neutral finding and resolution paragraph shown up front and the itemized dollar-value diffs collapsed behind a "See details" toggle, and dismissed/hard-to-find items collapsed under a "Reviewed — no action needed" toggle. A marked-up copy of the justification document can be downloaded with problem values highlighted.
