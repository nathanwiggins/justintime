...
- A project remembers its spreadsheet, project documentation, template type, profile, Total Budget figure, and in-progress draft, so you can leave and come back later.
- Projects are stored entirely in your browser (IndexedDB) — nothing is uploaded to a server.
- Settings (API key and Institutional Profiles) live on the Projects screen, via the **Settings** button — they're shared across all your projects, not scoped to one.

### Justification
A single workflow with two ways in — generate a new draft, or bring your own:

- **Generate a New Draft** — a 3-phase flow: Generate a first draft, Validate it, then Edit it in an interactive canvas before exporting.
  ...
  - The spreadsheet's overall Total Budget figure is extracted automatically by AI — two independent readings have to agree before it's accepted, retried up to 5 times. If they still don't agree, you're asked to type it in yourself.
...
- **Upload an Existing Justification** — already have a `.docx`? Upload it with its spreadsheet (or reuse the active project's spreadsheet with one click) and click **Verify Budget**.
  ...
  - Ends with a plain-language summary, a downloadable marked-up copy with flagged values highlighted, and an **Open in Editor** button that imports the document into the same interactive editor as a generated draft — every dollar figure it can match to your spreadsheet becomes Linked automatically.
...
- **The editor**, however you arrived — every dollar figure is either **Linked** to a spreadsheet cell or **Calculated** as a sum of other values, and everything else is free text you can edit directly, section headers included.
  - Click any figure to see where it comes from — a linked figure shows its sheet name and cell reference, a calculated one shows how many values feed into it — then relink it, edit its formula, or unlink it. Click anywhere else to close the inspector.
...
- On an on-premises (Vandalizer) deployment, a `profiles/profiles.json` file shipped with that deployment supplies a shared Institutional Profile visible to everyone on it — shown in Settings as read-only ("Local Default"), and used as the default profile for every user automatically. See `profiles/README.md`.
...
### Uploading an existing justification: trust the spreadsheet, question the writing
...
### Generating a new draft: the spreadsheet is the only source of truth
...
│   └── styles.css
├── js/
│   ├── app.js              # Boot, project-aware wiring
...
│   ├── verifier-tab.js     # Upload-and-audit UI: file handling, orchestration, results, hand-off to Editor
...
2. Opening a project reveals the Justification workflow, scoped to that project's files and draft.
3. Click **Switch Project** in the tab bar at any time to return to the project list.

### Generating a budget justification
1. From the Projects screen, open **Settings**, save your Gemini API key, and create at least one Institutional Profile with your fringe/F&A boilerplate.
2. Open a project, select a profile and Grant Template Type, upload your budget file and project summary, then click **Generate Justification**.
3. Review the draft, then **Keep This Version** or **Generate New Version**.
4. In the editor, link or relink figures to spreadsheet cells, build calculated values, and edit any text directly — headings and labels included.
5. Click **Export to .docx** whenever you're ready — export again anytime as you keep editing.

### Bringing your own justification
1. In the same project, scroll to **Upload an Existing Justification**, upload the `.docx` and its spreadsheet (or reuse the project's spreadsheet with one click), and click **Verify Budget**.
...
See [How It Works](#how-it-works) for the strategy behind both paths.
