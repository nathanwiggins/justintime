...
### Verifier
- The active project's budget spreadsheet and its justification draft — the current state of the editor, snapshotted to `.docx` on the fly, whether or not you've exported yet — are filled in automatically the moment you switch to this tab; upload a different file for either one if you want to check something else instead. Click **Verify Budget** when ready.
