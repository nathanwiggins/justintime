  ...
  const registry = {
    ...
    'nsf': [
      ...
      },
      {
        ...
        prompt: 'List all non-senior personnel grouped by role category. NSF Section B covers unnamed role groups such as Postdoctoral Scholars, Graduate Students, Undergraduate Students, and Other Professionals. For each group provide: the role category name, the number of individuals in that group, effort_months_per_year (the annual effort per individual — this is the per-year value, NEVER the cumulative total across the project; e.g. if each graduate student works 12 calendar months per year, effort_months_per_year is 12 not 60), the effort type (Calendar, Academic, or Summer), and the base_rate_per_individual (the annual cost for one individual in year 1). Then compute the yearly_breakdown: for each budget year, multiply number_of_individuals by the applicable per-individual rate for that year (accounting for any salary escalation stated in the budget), giving the combined group cost for that year. Set total_cost to the sum of all yearly_breakdown costs. Write a concise narrative for each group describing their role in the project. Do NOT reference or apply the NSF two-month policy — it does not apply to Section B. If no other personnel are budgeted, return an empty array.',
        schema: {
          type: 'object',
