  ...
  const registry = {
    ...
    'nsf': [
      ...
      },
      {
        ...
        label:  'D. Equipment',
        fields: ['equipment'],
        prompt: 'List each category of equipment as a SEPARATE array entry. Do NOT group, combine, or aggregate multiple categories of equipment into a single array entry under any circumstances. Each entry must contain the specific item name for that single piece of equipment, its total cost summed cumulatively across all budget years, a yearly_breakdown array showing the cost for each year this item is purchased (if it is only purchased in certain years, only include those years), and an individual narrative justification explaining why that specific piece is essential for the proposed research and how the cost was derived (if applicable). If no equipment is budgeted, return an empty array.',
        schema: {
          type: 'object',
    ...
    'general': [
      ...
      },
      {
        ...
        fields: ['fringe_benefits'],
        prompt: 'Generate the fringe benefits justification using the institutional fringe rate context provided. Do NOT break the fringe calculation down by personnel category or rate group — apply the appropriate rate from the institutional context to each relevant personnel category\'s salary internally, then sum everything into a SINGLE combined figure. Provide a yearly_breakdown showing the combined fringe cost for each budget year (summed across all personnel categories), and set total_cost to the cumulative sum across all years. Write a concise narrative_description that explains which rates were applied to which personnel categories and why, explicitly referencing the specific percentages and dollar figures from the institutional context, even though the numeric total itself is not broken out by category.',
        schema: {
          type: 'object',
