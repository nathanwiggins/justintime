(1) UI Design Strategy:
    - Projects view and normal interface should not be visible concurrently. (Addressed: already true in the existing code — confirmed via ProjectPicker's show/hide toggling, no change needed.)
    - Settings tab should be in project view, not the normal interface. (Addressed: Settings moved out of the per-project tab bar to a "Settings" button on the Projects screen.)
    - The normal interface will ultimately not feature a separate "Generator" and "Verifier" tab. The two features will be interwoven. The user can generate a budget justification based on a budget spreadsheet and project document, or they can just upload a budget justification themselves. Our generation workflow generates a budget justification from a budget spreadsheet and project document. The verifier audits the budget justification against the budget spreadsheet. (Addressed: merged into one "Justification" tab with two entry points — Generate New Draft / Upload Existing — both now end up in the same interactive editor, with the upload path still going through the existing guided audit chat first.)
